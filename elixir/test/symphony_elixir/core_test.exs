defmodule SymphonyElixir.CoreTest do
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert is_map(hooks)
    assert Map.get(hooks, "after_create") =~ "SYMPHONY_PROJECT_REPO_URL"
    assert Map.get(hooks, "after_create") =~ "git clone --depth 1 \"$SYMPHONY_PROJECT_REPO_URL\" ."
    assert Map.get(hooks, "after_create") =~ "cd elixir && mise trust"
    assert Map.get(hooks, "after_create") =~ "mise exec -- mix deps.get"
    assert Map.get(hooks, "before_remove") =~ "cd elixir && mise exec -- mix workspace.before_remove"

    project_repos = Map.get(config, "project_repos", %{})
    assert is_map(project_repos)
    assert Map.get(project_repos, "Symphony") == "https://github.com/openai/symphony"
